...
/**
 * AiService - Anthropic Claude API integration for To-Do List App.
 * Uses claude-haiku-4-5 (fast & efficient) for task suggestions.
 * Setup: Set your API key in environment variable ANTHROPIC_API_KEY
 * or replace the placeholder below.
 */
public class AiService {

    // ⚠️ Ganti dengan API key kamu, atau set environment variable ANTHROPIC_API_KEY
    private static final String API_KEY = System.getenv("ANTHROPIC_API_KEY") != null
            ? System.getenv("ANTHROPIC_API_KEY")
            : "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";
    private static final String API_URL = "https://api.anthropic.com/v1/messages";
    private static final String MODEL = "claude-haiku-4-5-20251001";

    /**
     * Generate AI suggestion for a task based on title and description.
     */
    ...
    /**
     * Check whether a task should be marked as priority.
     * Returns "true", "false", or reasoning.
     */
    ...
    /**
     * Suggest a description for a new category.
     */
    public static String suggestKategoriDescription(String namaKategori) {
        String prompt = "Kamu adalah asisten produktivitas. " +
                ...
                "\"" + namaKategori + "\". " +
                "Deskripsi harus menjelaskan jenis tugas apa yang cocok masuk kategori ini. " +
    ...
    /**
     * Improve or complete a category description.
     */
    ...
    /**
     * Analyze all priority tasks and give a summary + recommendation.
     */
    ...
    /**
     * Core method: call Anthropic Claude API.
     */
    private static String callClaude(String userMessage) {
        ...
        try {
            ...
            if (responseCode != 200) {
                return "⚠️ Error API (" + responseCode + "): " + extractErrorMessage(response.toString());
            }

            return extractTextFromResponse(response.toString());

        } catch (MalformedURLException e) {
            return "⚠️ URL tidak valid: " + e.getMessage();
    ...
    private static String extractErrorMessage(String json) {
        ...
        if (errIdx == -1) return json.length() > 100 ? json.substring(0, 100) : json;
        int startQuote = json.indexOf("\"", errIdx + 10);
        if (startQuote == -1) return json.length() > 100 ? json.substring(0, 100) : json;
        int endQuote = json.indexOf("\"", startQuote + 1);
        if (endQuote == -1) return json.length() > 100 ? json.substring(0, 100) : json;
